...
class Vector2 {
	...
	add(rhs) {
		...
		else return new Vector2(this.x + rhs, this.y + rhs);
	}
	sub(rhs) {
		if (rhs instanceof Vector2)
			return new Vector2(this.x - rhs.x, this.y - rhs.y);
...
class GraphNode {
	//grafta node'un özelliklerinin atanması
...
class GraphEdge {
	//edge çizebilmek için mouse tutup bırakma yerlerinin belirlenmesi
	constructor(nodeIndex1, nodeIndex2, edgeWeight) {
		//ağırlık ekle
...
for (const dropdownButton of document.getElementsByClassName(
	"dropdown-button"
)) {
	dropdownButton.addEventListener("click", (event) => {
		var _a;
		(_a = dropdownButton.nextElementSibling) === null || _a === void 0
			? void 0
...
function closeDropdown(event) {
	...
	for (const dropdown of document.getElementsByClassName("dropdown")) {
		if (
			event === undefined ||
			!((_a = dropdown.parentNode) === null || _a === void 0
				? void 0
...
function randomHslColor(lightness = 0.5) {
	return hslToRgbHex(Math.random(), Math.random() * 0.5 + 0.25, lightness);
}
...
function importConfirm() {
	const textarea = document.getElementById("import");
	if (textarea.value) importJson(textarea.value);
...
function importJson(json) {
	...
	for (const node of nodes) {
		...
		if (!isNaN(label) && label > max) max = label;
	}
	labelCounter = max + 1;
	...
	for (let nodeIndex = 0; nodeIndex < nodes.length; nodeIndex++) {
		...
		if (distanceSqr < Math.pow(node.radius + defaultNodeRadius, 2)) {
			if (closestNodeIndex === -1) {
				...
				distanceSqr <
				Math.pow(position.x - closestNodeX, 2) +
					Math.pow(position.y - closestNodeY, 2)
			) {
				closestNodeIndex = nodeIndex;
				closestNodeX = node.position.x;
				closestNodeY = node.position.y;
...
function nodeRadiusCurve(radius) {
	//bu sanırım vertex boyu değiştirmek için
	...
	nodeIndices.forEach((nodeIndex) => {
		if (nodeIndex >= 0 && nodeIndex < nodes.length) {
			...
			throw new RangeError(
				`nodeIndex = ${nodeIndex} is out of range for nodes.length = ${nodes.length}`
			);
	});
}
function deleteNodes(nodeIndices) {
	new Int32Array(nodeIndices)
		.sort()
		.reverse()
		.forEach((nodeIndex) => deleteNode(nodeIndex));
	draw(window.performance.now());
}
function deleteNode(nodeIndex) {
	for (let i = edges.length - 1; i >= 0; i--) {
		...
		if (edge.nodeIndex1 > nodeIndex) edge.nodeIndex1--;
		if (edge.nodeIndex2 > nodeIndex) edge.nodeIndex2--;
	}
	removeItem(selectedNodeIndices, nodeIndex);
	for (let i = selectedNodeIndices.length - 1; i >= 0; i--) {
		if (selectedNodeIndices[i] > nodeIndex) selectedNodeIndices[i]--; // Shift selected node indices to adjust
	}
...
function mousedown(event) {
	let mousePosition = getPositionRelativeToElement(
		event.target,
		event.clientX,
		event.clientY
	);
			...
			if (event.buttons === 1) {
				...
				if (event.shiftKey && event.ctrlKey) {
					...
					if (mouseDownNodeIndex === -1) {
						...
						if (
							selectedNodeIndices.indexOf(mouseDownNodeIndex) ===
							-1
						) {
							addItemUnique(
								selectedNodeIndices,
	...
	switch (state) {
		case State.None:
			if (
				...
				nodes[lastMouseDownNodeIndex].position.sub(mousePosition)
					.magnitudeSqr >
					Math.pow(nodes[lastMouseDownNodeIndex].radius, 2)
			) {
		...
		case State.MoveNode:
			if (lastMouseDownNodeIndex !== -1) {
				...
				else moveNodes(movement, [lastMouseDownNodeIndex]);
			}
			break;
		case State.DeleteNode:
			if (mouseHoverNodeIndex !== -1) deleteNode(mouseHoverNodeIndex);
			break;
...
function mouseup(event) {
	...
	switch (state) {
		case State.None:
			if (
				lastMouseDownNodeIndex !== -1 &&
				lastMouseDownNodeIndex === mouseUpNodeIndex
			) {
				selectedNodeIndices = [];
		...
		case State.BoxSelect:
			...
			getNodeIndicesInRect(rect).forEach((nodeIndex) =>
				addItemUnique(selectedNodeIndices, nodeIndex)
			);
			break;
		case State.DrawNode:
			if (lastMouseDownPosition === null)
		...
		case State.DrawEdge:
			if (lastMouseDownNodeIndex !== -1 && mouseUpNodeIndex !== -1) {
				if (
					!edges.some(
						(edge) =>
							edge.nodeIndex1 === lastMouseDownNodeIndex &&
							edge.nodeIndex2 === mouseUpNodeIndex
					)
				)
					edges.push(
						new GraphEdge(
							lastMouseDownNodeIndex,
							mouseUpNodeIndex,
							getWeight()
						)
					); //ağırlık eklendi
		...
		case State.DeleteEdge:
			if (lastMouseDownPosition === null)
				throw new Error("State machine bug.");
			let mouseDeltaSqr = mousePosition.sub(
				lastMouseDownPosition
			).magnitudeSqr;
...
function touchstart(event) {
	...
	for (let i = 0; i < event.changedTouches.length; i++) {
		...
		let touchPosition = getPositionRelativeToElement(
			touch.target,
			touch.clientX,
			touch.clientY
		);
	...
	if (touchInfos.size === 1) {
		...
		switch (state) {
			case State.None:
				let doubleTap = false;
				if (
					lastSingleTouchStartPosition !== null &&
					lastSingleTouchStartTimestamp !== -1
				) {
					let doubleTapDistanceSqr = touchInfo.touchPosition.sub(
						lastSingleTouchStartPosition
					).magnitudeSqr;
					if (
						event.timeStamp - lastSingleTouchStartTimestamp <
							touchDoubleTapTimeout &&
						doubleTapDistanceSqr <
							Math.pow(touchDoubleTapDistanceThreshold, 2)
					) {
						doubleTap = true;
				...
				if (!doubleTap) {
					touchHoldTimer = setTimeout(function () {
						if (touchInfos.size === 1) {
							...
							if (touchInfo.touchStartNodeIndex !== -1) {
								if (
									selectedNodeIndices.indexOf(
										touchInfo.touchStartNodeIndex
									) === -1
								)
									selectedNodeIndices = [
										touchInfo.touchStartNodeIndex,
...
function touchmove(event) {
	for (let i = 0; i < event.changedTouches.length; i++) {
		...
		if (touchInfo === undefined) continue;
		...
		touchInfo.touchClientPosition = new Vector2(
			touch.clientX,
			touch.clientY
		);
	...
	if (touchInfos.size === 1) {
		...
		switch (state) {
			...
			case State.MoveNode:
				if (
					selectedNodeIndices.indexOf(
						lastSingleTouchStartNodeIndex
					) !== -1
				)
					moveNodes(touchInfo.touchDelta, selectedNodeIndices);
				else
					moveNodes(touchInfo.touchDelta, [
						lastSingleTouchStartNodeIndex,
					]);
				break;
			case State.DeleteNode:
				...
				break;
			case State.DeleteEdge:
				let touchOldPosition = touchInfo.touchPosition.sub(
					touchInfo.touchDelta
				);
		...
		switch (state) {
			case State.Pan:
				let deltaAvg = touchInfo1.touchDelta
					.add(touchInfo2.touchDelta)
					.div(2);
		...
		switch (state) {
			...
			case State.DrawNode:
				if (lastSingleTouchStartPosition === null)
					throw new Error("State machine bug.");
				let nodeRadius = touchInfo.touchPosition.sub(
					lastSingleTouchStartPosition
				).magnitude;
				nodes.push(
					new GraphNode(
			...
			case State.DrawEdge:
				if (
					lastSingleTouchStartNodeIndex !== -1 &&
					touchInfo.touchOnNodeIndex !== -1
				) {
					if (
						!edges.some(
							(edge) =>
								edge.nodeIndex1 ===
									lastSingleTouchStartNodeIndex &&
								edge.nodeIndex2 === touchInfo.touchOnNodeIndex
						)
					)
						edges.push(
							new GraphEdge(
								lastSingleTouchStartNodeIndex,
								touchInfo.touchOnNodeIndex,
								5
							)
						); //ağırlık ekle
				}
			...
			case State.BoxSelect:
				...
				const box = {
					left: Math.min(
						lastSingleTouchStartPosition.x,
						touchInfo.touchPosition.x
					),
					top: Math.min(
						lastSingleTouchStartPosition.y,
						touchInfo.touchPosition.y
					),
					right: Math.max(
						lastSingleTouchStartPosition.x,
						touchInfo.touchPosition.x
...
function draw(timeStamp) {
	let deltaTime =
		lastDrawTimestamp === -1 ? 0 : lastDrawTimestamp - timeStamp;
	if (touchEnabled) {
	...
	try {
		...
		if (
			...
			ctx.beginPath();
			ctx.moveTo(
				nodes[lastMouseDownNodeIndex].position.x,
				nodes[lastMouseDownNodeIndex].position.y
			);
		...
		for (let i = 0; i < edges.length; i++) {
			...
			ctx.fillText(
				edge.edgeWeight,
				((node1.position.x + node2.position.x) / 2) + 5, // added 5px cause not on it, but a little to the edge of the edge
				((node1.position.y + node2.position.y) / 2) + 5
			);
		}
		...
		for (const edge of highlightedEdges) {
			...
			ctx.beginPath();
			ctx.moveTo(
				nodes[edge.nodeIndex1].position.x,
				nodes[edge.nodeIndex1].position.y
			);
			ctx.lineTo(
				nodes[edge.nodeIndex2].position.x,
				nodes[edge.nodeIndex2].position.y
			);
			ctx.stroke();
		...
		if (state === State.DrawNode) {
			if (lastMousePosition === null || lastMouseDownPosition === null)
				throw new Error(
					"lastMousePosition or lastMouseDownPosition cannot be null."
				);
			let nodeRadius = lastMousePosition.sub(
				lastMouseDownPosition
			).magnitude;
