...
function findEdgeWeight(index1, index2) {
    let we = -1;
...
function removeAlgorithmObjects() {
    let div = document.querySelector(".div-element");
    if (div !== null) {
        div.remove();
    }
    let div_total = document.querySelector(".total-div");
    if (div !== null) {
        div.remove();
    }
}
...
function drawRandomGraph(size) {
    ...
    for (let i = 1; i < size; i++) {
        nodes.push(new GraphNode(new Vector2(Math.floor(200 + Math.random() * 700), Math.floor(150 + Math.random() * 450)), 25, randomHslColor(), i));
    }
...
//# sourceMappingURL=mhmt_ops.js.map